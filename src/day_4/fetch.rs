...
{
    include_str!("../../inputs/4.txt").lines().map(|r| calculate_pair_from_string(r)).collect()
}
...
fn calculate_pair_from_string(input: &str) -> Pair
{
    ...
    if ! re.is_match(input)
    {
        ...
        for item in re.captures_iter(input) {
            pair = Pair {
                range_1: (item[1].parse::<u8>().unwrap()..item[2].parse::<u8>().unwrap() + 1).collect(),
                range_2: (item[3].parse::<u8>().unwrap()..item[4].parse::<u8>().unwrap() + 1).collect()
            };
        }
        pair
    }
...
fn test_calculate_pair_from_string() {
    ...
    assert_eq!(Pair {range_1: Vec::from([1,2]), range_2: Vec::from([5,6,7])}, result)
}
