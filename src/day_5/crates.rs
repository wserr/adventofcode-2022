...
pub fn move_crates_multiple<'a>(
    stacks: &'a mut Vec<VecDeque<String>>,
    move_operations: &Vec<MoveOperation>,
    multiple_crates_at_once: bool,
) -> Result<&'a mut Vec<VecDeque<String>>, &'static str> {
...
fn move_crates<'a>(
    ...
    while let Some(result) = if multiple_crates_at_once {
        items_to_push.pop_back()
    } else {
        items_to_push.pop_front()
    } {
...
fn test_move_crates_multiple_at_once() {
    let mut crates = Crates {
        stacks: Vec::from([
            VecDeque::from([str("A"), str("B"), str("C")]),
            VecDeque::from([str("D")]),
            VecDeque::from([str("E"), str("F"), str("G")]),
        ]),
    };
    ...
    assert_eq!(
        Ok(&mut Vec::from([
            VecDeque::from([str("A")]),
            VecDeque::from([str("D"), str("B")]),
            VecDeque::from([str("E"), str("F"), str("G"), str("C")])
        ])),
        result
    );
}
...
fn test_move_crates_multiple() {
    ...
    assert_eq!(
        Ok(&mut Vec::from([
            VecDeque::from([]),
            VecDeque::from([str("D"), str("C"), str("F"), str("G"), str("B"), str("A")]),
            VecDeque::from([str("E")])
        ])),
        result
    );
...
fn test_move_crates_success() {
    ...
    assert_eq!(
        Vec::from([
            VecDeque::from([str("A"), str("B")]),
            VecDeque::from([str("D"), str("C")])
        ]),
        crates.stacks
    )
}
