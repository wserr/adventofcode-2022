...
use super::crates::{MoveOperation, Crates};
use regex::Regex;

fn fetch_input() -> Vec<String>
{
    include_str!("../../inputs/5.txt").lines().map(|r| r.to_string()).collect()
}
...
pub fn fetch_moves() -> Vec<MoveOperation>
{
    ...
    input_drained.into_iter().map(|x| fetch_move_operation_from_str(&x)).collect()
}
