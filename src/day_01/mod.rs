use self::{elf::TotalCalorieCountCalculator, elves::MostWantedElfCalculator};
...
pub fn solution_1() -> String {
    ...
    match result.first() {
        Some(res) => format!(
            "Elf {:?} has the most calories, more specifically, {:?}.",
            res.index,
            res.calculate_total_calorie_count()
        ),
...
pub fn solution_2() -> String {
    ...
    let sum: usize = result
        .iter()
        .map(|x| x.calculate_total_calorie_count())
        .sum();
